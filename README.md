...
### Vercel Hosted Server Configuration

If you are using the hosted version of this server on Vercel, configure your MCP client as follows:

```json
{
  "mcpServers": {
    "clarity": {
      "url": "https://clarity-mcp-server.vercel.app/api/mcp",
      "headers": {
...
Replace `<your-token>` with the JWT token generated by `scripts/generate-token.js`.
